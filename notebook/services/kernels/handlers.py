...
class ZMQChannelsHandler(AuthenticatedZMQStreamHandler):
    ...
    def open(self, kernel_id):
        ...
        # kernel=self.kernel_manager.get_kernel(self.kernel_id)
        if km.kernel_ws.get(self.kernel_id) is None:
            km.kernel_ws[self.kernel_id]={self.user:[self.ws_connection]}
        elif km.kernel_ws[self.kernel_id].get(self.user) is None:
            km.kernel_ws[self.kernel_id][self.user]=[self.ws_connection]
        elif self.ws_connection not in km.kernel_ws[self.kernel_id].get(self.user):
            km.kernel_ws[self.kernel_id][self.user].append(self.ws_connection)
    ...
    def ws_sendmsgs(self,msg):
        ...
        reply['channel'] = 'iopub'
        # kernel = self.kernel_manager.get_kernel(self.kernel_id)
        for users in self.kernel_manager.kernel_ws.get(self.kernel_id):
            for ws in self.kernel_manager.kernel_ws[self.kernel_id].get(users):
    ...
    def on_message(self, msg):
        ...
        if msg.get("header") and msg.get("content"):
            if msg.get("header").get("msg_type") == "comm_msg" and msg.get("content").get("data").get("cellListId"):
                ...
                if self.kernel_manager.kernel_lock[self.kernel_id].locked():
                    print("kernel locked")
                    busy_msg = self.session.msg('comm_msg', content={"comm_id": 'x', "data": {"execute": "busy"}})
                    busy_msg['channel'] = 'iopub'
                    self.write_message(json.dumps(busy_msg, default=date_default))
                else:
                    print("receive set cellListId")
                    self.kernel_manager.kernel_lock[self.kernel_id].acquire()
                    self.kernel_manager.kernel_cell[self.kernel_id]=content.get("cellListId")
                    self.kernel_manager.kernel_cellnum[self.kernel_id]=int(content.get("cellnum"))
                return

        if msg.get("header"):
            if msg.get("header").get("msg_type") == "execute_request":
                print(msg.get("metadata").get("cellListId"),self.kernel_manager.kernel_cell.get(self.kernel_id))
                if self.kernel_manager.kernel_cell.get(self.kernel_id)==msg.get("metadata").get("cellListId"):
                    ...
                    return
        # else:
        #     print('unlocked user can execute')
        #     self.kernel_manager.kernel_lock[self.kernel_id].acquire()
        #     headers = msg.get("header")
        #     self.kernel_manager.kernel_user[self.kernel_id] = headers.get("session")
        #     self.kernel_manager.kernel_cell[self.kernel_id] = 1
        #     metadata = msg.get("metadata")
        #     exec_msg = self.session.msg('comm_msg', content={"comm_id": "x",
        #                                                      "data": {"cellid": metadata.get('id'),
        ...
        #     exec_msg['channel'] = 'iopub'
        #     for users in self.kernel_manager.kernel_ws.get(self.kernel_id):
        #         if users != headers.get('username'):
        #             for ws in self.kernel_manager.kernel_ws[self.kernel_id].get(users):
        #                 if ws is None or ws.is_closing():
        #                     continue
        #                 ws.write_message(json.dumps(exec_msg, default=date_default))
                # exec_msg = self.session.msg('comm_msg', content={"data":{"cellid":msg.metadata.id}})
                # exec_msg['channel'] = 'iopub'
                # # kernel = self.kernel_manager.get_kernel(self.kernel_id)
                # for users in self.kernel_manager.kernel_ws.get(self.kernel_id):
                #     for ws in self.kernel_manager.kernel_ws[self.kernel_id].get(users):
        ...
        # elif mt=='comm_msg':
        #     print('收到请求准备广播')
        #     # reply=self.session.msg('display_data',content=msg['content'])
        #     # reply['channel']='iopub'
        #     # self.write_message((json.dumps(reply, default=date_default)))
        else:
            stream = self.channels[channel]
    ...
    def _on_zmq_reply(self, stream, msg_list):
        ...
        def write_stderr(error_message):
            ...
            msg = self.session.msg("stream",
                content={"text": error_message + '\n', "name": "stderr",'metadata':'','transient':''},
                parent=parent,
                metadata={},
            )
            msg['channel'] = 'iopub'
        ...
        if msg.get("header"):
            if msg.get("header").get("msg_type") == "execute_reply":
                ...
                self.kernel_manager.kernel_cellnum[self.kernel_id]=self.kernel_manager.kernel_cellnum[self.kernel_id]-1
                if self.kernel_manager.kernel_cellnum[self.kernel_id]==0:
                    self.kernel_manager.kernel_lock[self.kernel_id].release()

                # exec_count=content_re.get('execution_count')
                # exec_msg = self.session.msg('comm_msg', content={"comm_id":"x","data":{"cellid":"","func":"sync","spec":"exec_reply","execution_count":exec_count}})
                # exec_msg['channel'] = 'iopub'
                # # kernel = self.kernel_manager.get_kernel(self.kernel_id)
                # for users in self.kernel_manager.kernel_ws.get(self.kernel_id):
                #     for ws in self.kernel_manager.kernel_ws[self.kernel_id].get(users):
    ...
    def close(self):
        self.kernel_manager.kernel_ws.get(self.kernel_id).remove(self.ws_connection)
